...
app = FastAPI(title="JobIA Backend", version=APP_VERSION, description="Employment intelligence module of Bitey IA, exposed through JobIA web and Android channels.")
...
def overlaps(first: str, second: str) -> bool:
    ...
    if normalized_first in normalized_second or normalized_second in normalized_first: return True
    first_tokens = {token for token in normalized_first.replace("+", " + ").replace("#", " # ").replace(".", " . ").split() if len(token) > 2}
    second_tokens = {token for token in normalized_second.replace("+", " + ").replace("#", " # ").replace(".", " . ").split() if len(token) > 2}
...
def profile_from_row(row):
    if row is None: return Profile()
    try: skills = json.loads(row["skills"]); skills = skills if isinstance(skills, list) else []
...
def prepare_application(job: Job, profile: Profile) -> ApplicationDrafts:
    ...
    return ApplicationDrafts(cvSummary=f"{profession} with results-oriented experience and knowledge in {skill_text}. Interested in {modality.lower()} opportunities and applying technical skills, problem solving, and continuous learning.{match_text}", coverLetter=f"Hello,\n\nI am interested in the {job.title} opportunity at {job.company}. My profile as a {profession.lower()} and my knowledge of {skill_text} allow me to contribute technical capability, analysis, and results orientation.\n\nI would be glad to discuss my experience and how I can contribute to the team.\n\nRegards,\n{profile.email or 'Candidate'}", answers=f"Motivation: I am interested in {job.title} because it connects my experience as a {profession.lower()} with the opportunity to create value at {job.company}.\n\nRelevant strengths: {skill_text}." + (f"\n\nDirect matches: {', '.join(matched)}." if matched else "") + f"\n\nAvailability: {modality}.", notes="Draft generated by JobIA Backend. Review personal data, requirements, experience, and conditions before authorizing any external action.")
...
def execute_delegation(request: DelegationRequest) -> dict[str, Any]:
    ...
    if intent == "application_preparation":
        ...
        if not job_match:
            return {**base, "execution_status": "needs_input", "result_type": "application", "result": {"required": ["job_id", "profile"], "hint": "Specify the JobIA opportunity and profile before preparing application materials."}, "answer": "Puedo preparar la postulación, pero necesito identificar la vacante y el perfil laboral."}
        return {**base, "execution_status": "needs_input", "result_type": "application", "result": {"required": ["profile"], "job_id": f"jobia-{job_match.group(1)}"}, "answer": "La vacante está identificada; necesito el perfil laboral para generar una postulación personalizada."}
...
@app.get("/health")
def health(): return {"status": "ok", "service": "jobia", "version": APP_VERSION, "contract": API_CONTRACT, "persistence": "sqlite"}

@app.get("/api/v1/capabilities")
def capabilities(): return {"module": "jobia", "parent": PARENT_MODULE, "host_channel": HOST_CHANNEL, "capabilities": ["opportunities", "matching", "profiles", "applications", "alerts"], "api_version": API_CONTRACT, "trainer": TRAINER_MODULE}
@app.get("/api/v1/module/status")
def module_status(): return {"module": "JobIA", "parent": "Bitey IA", "host_channel": "Bitey IA Web", "status": "ready", "contract": API_CONTRACT, "trainer": {"module": TRAINER_MODULE, "role": "training-and-validation"}}
@app.get("/api/v1/cognitive/status")
def cognitive_status(): return {"status": "delegated", "owner": "Bitey IA", "host_channel": "Bitey IA Web", "module": "JobIA", "mode": "employment-intelligence", "bidirectional": True, "inbound": "Bitey IA may delegate employment capabilities to JobIA", "outbound": "JobIA may request general capabilities from Bitey IA"}
...
def contract(): return {"name": API_CONTRACT, "module": "JobIA", "parent_system": "Bitey IA", "specialization": "employment-and-work", "host_channel": "Bitey IA Web", "trainer": TRAINER_MODULE, "clients": ["JobIA-Web", "JobIA-app"], "principle": "Bitey IA coordinates; JobIA executes employment specialization; Bitey Trainer trains and validates."}
